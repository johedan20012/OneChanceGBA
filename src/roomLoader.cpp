...
bn::unique_ptr<Room> RoomLoader::loadRoom(RoomExit room_exit,Player& player,GlobalVariables& _global){
    ...
    if(room_exit.name == roomsNames[3]){
        return bn::make_unique<HouseBathroom>(player,room_exit.next_in_dir,_global);
    }
    if(room_exit.name == roomsNames[4]){
        return bn::make_unique<HouseEntrance>(player,room_exit.next_in_dir,_global);
    }
    if(room_exit.name == roomsNames[5]){
        return bn::make_unique<City>(player,room_exit.next_in_dir,_global);
    }
    ...
    if(room_exit.name == roomsNames[8]){
        return bn::make_unique<WorkHall>(player,room_exit.next_in_dir,_global);
    }
    if(room_exit.name == roomsNames[9]){
        return bn::make_unique<WorkLabDoor>(player,room_exit.next_in_dir,_global);
    }
    if(room_exit.name == roomsNames[10]){
        return bn::make_unique<WorkStairs>(player,room_exit.next_in_dir,_global);
    }
