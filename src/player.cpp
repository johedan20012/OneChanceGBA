...
void Player::takeOffLabCoat(){
    lab_coat = false;
    sprite.set_tiles(bn::sprite_items::character.tiles_item().create_tiles(5));   
    walking = bn::create_sprite_animate_action_forever(sprite,6,bn::sprite_items::character.tiles_item(),7,6);
}

void Player::resetAnim(){
    sprite.set_tiles(bn::sprite_items::character.tiles_item().create_tiles(0+ ((!lab_coat)? 5:0) ));
}
...
void Player::bend(){
    ...
    sprite.set_tiles(bn::sprite_items::character.tiles_item().create_tiles(3+ ((!lab_coat)? 5:0) ));
...
void Player::update(bool frozen){
    ...
    if(state == STATE::BENDING){
        ...
        if(bend_stand_timer->elapsedFrames() == 14) sprite.set_tiles(bn::sprite_items::character.tiles_item().create_tiles(4+ ((!lab_coat)? 5:0) ));
    ...
    if(prev_mov_dir != 0 && moving_dir == 0) sprite.set_tiles(bn::sprite_items::character.tiles_item().create_tiles(0+ ((!lab_coat)? 5:0) ));
