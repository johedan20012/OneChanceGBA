...
# ROMCODE is a uppercase ASCII, max 4 characters text string containing the output ROM code. DON'T MODIFY IT.
...
PYTHON      :=  python
SOURCES     :=  src src/rooms
INCLUDES    :=  include include/rooms
DATA        :=
GRAPHICS    :=  graphics graphics/bgs graphics/sprites
AUDIO       :=  audio
DMGAUDIO    :=  dmg_audio
ROMTITLE    :=  ONE CHANCE
ROMCODE     :=  SBTP
USERFLAGS   :=  -DDEBUG_GAME
USERASFLAGS :=  
USERLDFLAGS :=  
USERLIBDIRS :=  
USERLIBS    :=  
